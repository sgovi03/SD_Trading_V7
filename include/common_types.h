...
enum class ZoneState {
    ...
    EXHAUSTED       // Zone produced N consecutive losses — blocked until it wins or is reset
};
...
struct Zone {
    ...
    // ⭐ EXHAUSTION: Per-zone consecutive-loss counter
...
public:
    ...
    // ============================================================
    // ⭐ NEW FILTERS — from live trade analysis (March 2026)
    // ============================================================

    // Priority 1: Zone consecutive-loss exhaustion
    // After zone_consecutive_loss_max consecutive SL losses from the same zone,
    // mark it EXHAUSTED. Blocks re-entry until a TP win resets the counter.
    ...
    // Separate from entry_block_start_time (which blocks early open period).
    bool        enable_late_entry_cutoff = false;   // Master toggle (tune per live session)
    std::string late_entry_cutoff_time   = "14:30"; // HH:MM — block new entries at/after this

    // Priority 4: Hard cap on position lots (applied after all dynamic sizing)
    // 6-lot trade: -₹16,164 (worst single loss); 4-lot trades: 78% WR, avg +₹12,326.
    // Applied as final ceiling AFTER max_loss_per_trade has already reduced size.
