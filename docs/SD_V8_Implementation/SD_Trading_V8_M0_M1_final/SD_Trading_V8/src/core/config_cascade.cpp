...
ResolvedConfig ConfigCascadeResolver::resolve(
    ...
    if (!file_exists(master_path)) {
        ...
        // During migration, callers may not have created the new structure yet.
        ...
        // Caller must handle this by passing the old config path as config_root
        result.report.used_fallback_mode = true;
        result.report.fallback_path      = config_root;
        result.config = ConfigLoader::load_from_file(config_root);
