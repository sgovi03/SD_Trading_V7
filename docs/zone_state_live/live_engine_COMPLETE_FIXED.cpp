...
void LiveEngine::wire_v6_dependencies() {
    ...
    // ⭐ FIX #2: Initialize Zone State Manager
    zone_state_manager_ = std::make_unique<Core::ZoneStateManager>(config);
    LOG_INFO("✅ Zone State Manager initialized (Live Mode)");
...
bool LiveEngine::try_load_backtest_zones() {
    ...
    // ⭐ FIX #2: Initialize zone states after loading
    if (zone_state_manager_ && !active_zones.empty()) {
        zone_state_manager_->initialize_zone_states(active_zones);
        LOG_INFO("Zone states initialized for " << active_zones.size() << " loaded zones");
    }
