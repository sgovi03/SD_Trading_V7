...
BacktestEngine::BacktestEngine(
    ...
    if (config.v6_fully_enabled) {
        ...
        if (volume_baseline_.load_from_file(baseline_path)) {
            ...
            zone_state_manager_ = new Core::ZoneStateManager(config);
            LOG_INFO("✅ Zone State Manager initialized (Backtest Mode)");
...
BacktestEngine::~BacktestEngine() {
    ...
    if (zone_state_manager_ != nullptr) {
        delete zone_state_manager_;
        zone_state_manager_ = nullptr;
    }
...
void BacktestEngine::update_zone_states(const Core::Bar& bar) {
    // ⭐ FIX #2: Call Zone State Manager FIRST
    if (zone_state_manager_ && !active_zones.empty()) {
        zone_state_manager_->update_zone_states(
            active_zones, 
            bar, 
