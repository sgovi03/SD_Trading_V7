...
// ⭐ CRITICAL FIX: Much stricter state freshness scoring
double ZoneScorer::calculate_state_freshness_score(const Zone& zone) const {
    ...
    switch (zone.state) {
        ...
        case ZoneState::TESTED:
            ...
            // TESTED zones should be significantly less valuable than FRESH
            base_score = max_score * 0.50;  
            break;
    ...
    // ⭐ FIX: Apply MUCH stricter touch count penalty
    // Fresh zones (0-3 touches) should dominate
    double touch_penalty = 1.0;
    
    if (zone.touch_count <= 3) {
        // FRESH zones with 0-3 touches: NO penalty (100%)
        touch_penalty = 1.0;
    } else if (zone.touch_count <= 5) {
        // Lightly tested: minimal penalty (90%)
        touch_penalty = 0.90;
    } else if (zone.touch_count <= 10) {
        // Moderately tested: 70% remaining
        touch_penalty = 0.70;
    } else if (zone.touch_count <= 20) {
        // Well tested: 40% remaining
        touch_penalty = 0.40;
    } else if (zone.touch_count <= 30) {
        // Over-tested: 20% remaining
        touch_penalty = 0.20;
    } else if (zone.touch_count <= 50) {
        // Exhausted: 10% remaining
        touch_penalty = 0.10;
    } else {
        // Should be retired: 2% remaining (effectively disabled)
        touch_penalty = 0.02;
    }
    
    double final_score = base_score * touch_penalty;
    
    LOG_DEBUG("State freshness: state=" + std::to_string(static_cast<int>(zone.state)) +
              ", touches=" + std::to_string(zone.touch_count) +
              ", base=" + std::to_string(base_score) +
              ", penalty=" + std::to_string(touch_penalty) +
              ", final=" + std::to_string(final_score));
...
ZoneScore ZoneScorer::evaluate_zone(const Zone& zone, MarketRegime regime, const Bar& current_bar) const {
    ...
    LOG_DEBUG("Zone #" + std::to_string(zone.zone_id) + " scored: " +
              "total=" + std::to_string(score.total_score) + 
              ", touches=" + std::to_string(zone.touch_count) +
              ", state_fresh=" + std::to_string(score.state_freshness_score) +
