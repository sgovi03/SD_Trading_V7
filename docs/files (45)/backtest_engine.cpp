...
void BacktestEngine::manage_open_position(const Core::Bar& bar, int bar_index) {
    ...
    if (config.use_trailing_stop) {
        ...
        if (current_r >= 3.0) {
            ...
            // Stage 2: Lock 0.5R profit floor (direction-aware)
            // LONG:  SL moves to entry + 0.5R (above entry = profit protected)
            // SHORT: SL moves to entry - 0.5R (below entry = profit protected)
            new_trail_stop = (trade.direction == "LONG")
