...
SEARCH_SPACE: Dict[str, dict] = {
    ...
    "entry_minimum_score": {
        "type": "float", "min": 15.0, "max": 35.0, "step": 5.0,
    ...
    "entry_score_total_min": {
        "type": "int", "min": 80, "max": 140, "step": 10,
    ...
    "max_zone_age_days": {
        "type": "int", "min": 60, "max": 365, "step": 30,
...
class ResultParser:
    """
    Parse backtest output into BacktestResult metrics.
    Supports: CSV file, stdout text, or log file with key=value / tabular patterns.
    Extend parse_stdout() and parse_csv() to match your engine's actual output.
    """

    # Patterns for stdout/log parsing (regex → metric_name)
    STDOUT_PATTERNS = {
        "net_pnl":        r"(?:Net\s+P&L|Total\s+PnL|Net\s+Profit)\s*[=:]\s*([-\d,.]+)",
        "win_rate":       r"Win\s+Rate\s*[=:]\s*([\d.]+)\s*%?",
        "total_trades":   r"Total\s+Trades\s*[=:]\s*(\d+)",
        "profit_factor":  r"Profit\s+Factor\s*[=:]\s*([\d.]+)",
        "sharpe_ratio":   r"Sharpe\s+(?:Ratio)?\s*[=:]\s*([-\d.]+)",
        "max_drawdown_pct": r"Max\s+(?:Draw)?[Dd]own\s*[=:]\s*([\d.]+)\s*%?",
        "avg_win":        r"Avg\s+Win\s*[=:]\s*([-\d,.]+)",
        "avg_loss":       r"Avg\s+Loss\s*[=:]\s*([-\d,.]+)",
        "avg_rr_achieved": r"Avg\s+R[Rr]\s*[=:]\s*([-\d.]+)",
    }

    CSV_METRIC_COLUMNS = {
        "net_pnl":        ["net_pnl", "total_pnl", "net_profit"],
        "win_rate":       ["win_rate", "win_pct"],
        "total_trades":   ["total_trades", "trades"],
        "profit_factor":  ["profit_factor", "pf"],
        "sharpe_ratio":   ["sharpe", "sharpe_ratio"],
        "max_drawdown_pct": ["max_drawdown", "drawdown_pct"],
    }
    ...
    def parse(self, stdout: str, result_dir: Path) -> Dict[str, float]:
        ...
        for csv_file in result_dir.glob("*.csv"):
            csv_metrics = self._parse_csv(csv_file)
            if csv_metrics:
                metrics.update(csv_metrics)
                break
        # Then overlay stdout patterns
        stdout_metrics = self._parse_stdout(stdout)
        metrics.update(stdout_metrics)
        return metrics

    def _parse_stdout(self, text: str) -> Dict[str, float]:
        result = {}
        for metric, pattern in self.STDOUT_PATTERNS.items():
    ...
    def _parse_csv(self, csv_path: Path) -> Dict[str, float]:
        result = {}
        try:
            ...
            if not rows:
                return result
            # Check if summary row (last row or specific row)
            summary = rows[-1]
            headers_lower = {k.lower().strip(): k for k in summary.keys()}
            for metric, candidates in self.CSV_METRIC_COLUMNS.items():
                for c in candidates:
                    if c in headers_lower:
                        try:
                            val = summary[headers_lower[c]].replace(",", "")
...
class ObjectiveCalculator:
    ...
    def _composite(self, r: BacktestResult) -> float:
        ...
        Tuned for Indian NIFTY futures (₹ denominated).
        """
        score = 0.0
    ...
    def _risk_adjusted(self, r: BacktestResult) -> float:
        """Return on max drawdown × win rate — focuses on capital preservation."""
...
class BacktestRunner:
    ...
    def run(self, params: Dict[str, Any], config_mgr: ConfigManager,
        ...
        config_mgr.generate_config(params, config_path)
        ...
        if self.dry_run:
            # Simulate realistic-looking results for testing
        ...
        cmd = self.cmd_template.replace("{config}", str(config_path))
        t0 = time.time()
        try:
            proc = subprocess.run(
                cmd, shell=True, capture_output=True, text=True,
                timeout=self.timeout, cwd=str(run_dir)
            )
            result.duration_sec = time.time() - t0
            result.raw_output = proc.stdout + proc.stderr

            # Save raw output
            (run_dir / "backtest_output.txt").write_text(result.raw_output, encoding="utf-8")
...
class ResultsManager:
    ...
    def _generate_report(self, r: BacktestResult) -> str:
        lines = [
            ...
            f"  Avg Win          : ₹{r.avg_win:,.0f}",
            f"  Avg Loss         : ₹{r.avg_loss:,.0f}",
            "",
...
class Optimizer:
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        ch.setFormatter(fmt)
        log.addHandler(ch)
        fh = logging.FileHandler(self.output_dir / "optimizer.log")
        fh.setFormatter(fmt)
        log.addHandler(fh)
        return log
    ...
    def _run_and_score(self, params: Dict[str, Any], run_n: int) -> BacktestResult:
        ...
        if result.score > self.state.best_score:
            ...
            self.logger.info(
                ...
                f"pnl=₹{result.net_pnl:,.0f}  wr={result.win_rate:.1f}%  "
    ...
    def _final_report(self):
        ...
        self.logger.info("=" * 65)
        self.logger.info(f"{'Rank':>4}  {'Score':>9}  {'P&L (₹)':>12}  {'WR%':>6}  {'PF':>5}  {'DD%':>5}  {'Trades':>6}")
        self.logger.info("-" * 65)
