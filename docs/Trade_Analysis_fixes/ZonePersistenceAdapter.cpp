...
static ZoneState parse_zone_state(const Json::Value& state_val) {
    if (state_val.isString()) {
        ...
        if (s == "EXHAUSTED") return ZoneState::EXHAUSTED;  // NEW
    }
...
static void append_zone_json(std::stringstream& ss, const Zone& zone) {
    ss << "    {\n"
       ...
       << "      \"consecutive_losses\": " << zone.consecutive_losses << ",\n"  // NEW
       << "      \"exhausted_at_datetime\": \"" << zone.exhausted_at_datetime << "\",\n"  // NEW
       << "      \"swing_analysis\": {\n"
...
static void parse_zone_json(const Json::Value& zone_json, Zone& zone, bool force_active_true = false) {
    ...
    zone.consecutive_losses = zone_json.get("consecutive_losses", 0).asInt();  // NEW
    zone.exhausted_at_datetime = zone_json.get("exhausted_at_datetime", "").asString();  // NEW
