...
public:
    ...
    void publish(const EventType& event) {
        ...
        // ASYNC subscribers — build work items under sub_mutex_,
        // then push to queue under queue_mutex_ (never nested).
        // This eliminates the lock-order inversion that could deadlock
        // if the async worker ever called publish() re-entrantly.
        {
            std::vector<AsyncWorkItem> items;
            {
                std::lock_guard<std::mutex> lock(sub_mutex_);
                auto ait = async_subs_.find(key);
                if (ait == async_subs_.end()) return;
                if (ait->second.empty()) return;
                items.reserve(ait->second.size());
                for (auto& sub : ait->second) {
                    auto event_copy = event;
                    std::string sub_name = sub.name;
                    auto fn_copy = sub.fn;
                    items.push_back({ [event_copy, sub_name, fn_copy]() {
            ...
            }  // sub_mutex_ released here

            // Now push under queue_mutex_ only — no nested lock
            {
                std::lock_guard<std::mutex> qlock(queue_mutex_);
                for (auto& item : items)
                    async_queue_.push(std::move(item));
            }
