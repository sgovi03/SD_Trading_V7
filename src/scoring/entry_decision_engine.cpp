...
EntryDecision EntryDecisionEngine::calculate_entry(const Zone& zone, const ZoneScore& score, double atr, MarketRegime regime, const ZoneQualityScore* zone_quality_score, const Bar* current_bar, const std::vector<Bar>* bar_history, bool is_continuation) const {
   ...
   /* if (current_bar != nullptr) {
        ...
        if (zone.type == ZoneType::DEMAND) {
            // For DEMAND: measure from proximal (bottom) to current close
            penetration = (current_bar->close - zone.proximal_line) / zone_range;
        } else {
            // For SUPPLY: measure from proximal (top) to current close
        ...
        if (penetration > 0.5) {
            ...
            decision.rejection_reason = "Deep penetration (" + 
                std::to_string(static_cast<int>(penetration * 100)) + "% into zone, max 50%)";
            LOG_INFO("PENETRATION REJECTED: " + decision.rejection_reason);
            return decision;
        }
    }
  */
