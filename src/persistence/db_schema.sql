...
INSERT OR IGNORE INTO symbol_registry (
    ...
    'NIFTY-FUT', 'Nifty 50 Futures', 'INDEX_FUTURES', 'NSE',
    65, 0.05, 60000,
    'THURSDAY', 'NSE:NIFTY26APRFUT', 'conf/symbols/NIFTY-FUT.config', 1
);
...
INSERT OR IGNORE INTO symbol_registry (
    ...
    'BANKNIFTY-FUT', 'Bank Nifty Futures', 'INDEX_FUTURES', 'NSE',
    30, 5.0, 45000,
    'WEDNESDAY', 'NSE:BANKNIFTY26APRFUT', 'conf/symbols/BANKNIFTY-FUT.config', 1
);
...
CREATE TABLE IF NOT EXISTS zones (
    id                      INTEGER     PRIMARY KEY AUTOINCREMENT,  -- DB surrogate key
    zone_id                 INTEGER     NOT NULL,              -- per-symbol local ID (Zone::zone_id)
    symbol                  TEXT        NOT NULL REFERENCES symbol_registry(symbol),
    ...
    -- Composite unique: zone_id is per-symbol local counter, NOT globally unique
    UNIQUE (zone_id, symbol)
);
...
CREATE TABLE IF NOT EXISTS signals (
    ...
    zone_id                     INTEGER,   -- informational; zone_id+symbol identifies zone
    zone_score                  REAL,
...
VALUES ('NIFTY-FUT', date('now', 'localtime'), 1);

INSERT OR IGNORE INTO session_state (symbol, session_date, next_zone_id)
VALUES ('BANKNIFTY-FUT', date('now', 'localtime'), 1);
