...
void LiveEngine::update_bar_history() {
    ...
    // Strict phase boundary: once LIVE mode starts, do not accept HISTORICAL bars.
    // Late backfill during live feed can create out-of-order state and false detections.
    if (!historical_mode_ && latest_bar.source == "HISTORICAL") {
        late_historical_drop_count_++;
        LOG_DEBUG("[ZONE_MODE] Late HISTORICAL bar received after LIVE activation; ignoring: "
                  << latest_bar.datetime);
        return;
    }
    ...
    if (historical_mode_ && latest_bar.source != "HISTORICAL") {
        // If startup bootstrap ran before pipe historical bars arrived,
        // active_zones may still be empty. Run one-time detection now
        // over the completed HISTORICAL bar set, then switch to LIVE mode.
        if (active_zones.empty() && bar_history.size() >= 100) {
            LOG_INFO("[ZONE_MODE] HISTORICAL bootstrap completed from pipe: running one-time zone detection");
            active_zones = Core::ZoneInitializer::detect_initial_zones(bar_history, detector, next_zone_id_);
            LOG_INFO("[ZONE_MODE] One-time bootstrap detection found " << active_zones.size() << " zones");
        ...
        } else if (active_zones.empty()) {
            LOG_WARN("[ZONE_MODE] One-time bootstrap detection skipped: insufficient historical bars (" 
                     << bar_history.size() << ")");
        }
        ...
        LOG_INFO("  [ZONE_MODE] Incremental/periodic zone detection ENABLED");
    ...
    for (auto zone : new_zones) {
        ...
        if (!already_active) {
            ...
            ZoneStateEvent creation_event(
                ...
                bar_history.empty() ? zone.proximal_line : bar_history.back().high,
                bar_history.empty() ? zone.distal_line   : bar_history.back().low,
                0  // No touches yet
            );
...
void LiveEngine::check_for_entries() {
    ...
    if (bar_history.empty()) {
        LOG_DEBUG("[check_for_entries] bar_history empty — skipping.");
        return;
    }
...
std::stable_sort(active_zones.begin(), active_zones.end(), 
    ...
    for (auto& zone : active_zones) {
        ...
        // ⭐ FORMATION BAR GUARD: parity with V7 backtest_engine.cpp
        // if (bar_index <= zone.formation_bar) continue;
        // Prevents entry on the same bar a zone was just detected.
        {
            int current_bar_idx = static_cast<int>(bar_history.size()) - 1;
            if (current_bar_idx <= zone.formation_bar) {
                LOG_DEBUG("Zone " << zone.zone_id << " skipped: formation_bar="
                          << zone.formation_bar << " >= current_bar " << current_bar_idx);
                zones_rejected++;
                continue;
            }
        }
...
// If skip_retest = NO, allow VIOLATED zones to be evaluated
        ...
        if (distance_from_price > max_zone_distance) {
            zones_rejected++;
            LOG_DEBUG("Zone " << zone.zone_id << " skipped: too far"
                      " mid=" << zone_mid << " cur=" << current_price
                      << " dist=" << distance_from_price << " max=" << max_zone_distance);
            continue;
        }
        ...
        if (entered) {
            // ── ALWAYS assign order_tag, even in dry-run/simulation mode.
            // Previously tag was only generated when order_submitter was enabled,
            // so simulation runs stored order_tag="" in the DB causing ghost rows
            // and the TradeCloseListener infinite polling loop.
            {
                Trade& trade_ref = const_cast<Trade&>(trade_mgr.get_current_trade());
                if (trade_ref.order_tag.empty()) {
                    trade_ref.order_tag = OrderTagGenerator::generate(trade_ref.direction);
                }
            }

            // ── Submit to broker if enabled (live trading only) ──
            if (order_submitter_ && order_submitter_->is_enabled()) {
                Trade& trade_ref = const_cast<Trade&>(trade_mgr.get_current_trade());
                const std::string& tag = trade_ref.order_tag;
                ...
                if (!result.success) {
                    ...
                    LOG_INFO("OrderTag: " << tag << " Zone: " << zone.zone_id
                             << " Side: " << trade_ref.direction);
                }
            }
...
void LiveEngine::manage_position() {
    ...
    if (bar_history.empty()) {
        LOG_WARN("[manage_position] bar_history empty — skipping.");
        return;
    }
            ...
            for (auto& zone : active_zones) {
                ...
                if (exit_reason == "STOP_LOSS") {
                    ...
                    // (RC3 restore only works if the file has the updated count).
                    {
                        ...
                        meta["generated_at"] = current_bar.datetime;
                        meta["symbol"]       = trading_symbol;
                        zone_persistence_.save_zones_with_metadata(
                            active_zones, next_zone_id_, master_file_path.string(), meta);
                        LOG_INFO("[ZONE PERSIST] Saved zones after SL — zone "
                                  << zone.zone_id << " consecutive_losses="
                                  << zone.consecutive_losses);
                    }
                } else {
                    ...
                    if (is_winner && zone.consecutive_losses > 0) {
                        LOG_INFO("Zone " << zone.zone_id
                                 ...
                                 << exit_reason << " pnl=" << pnl << " — zone winner)");
                        zone.consecutive_losses = 0;
...
void LiveEngine::publish_trade_events(const Trade& t, const Zone& zone, bool is_close) {
    // Guard: never publish with empty order_tag — creates ghost DB rows
    // that cause TradeCloseListener to poll infinitely.
    if (t.order_tag.empty()) {
        LOG_WARN("[LiveEngine] publish_trade_events: empty order_tag skipped"
                 " (is_close=" << is_close << " symbol=" << trading_symbol << ")");
        return;
    }
    if (is_close) {
...
void LiveEngine::process_tick() {
    ...
    // Periodic feed-quality summary for ignored late HISTORICAL bars.
    if (!historical_mode_ && late_historical_drop_count_ > 0) {
        if (late_historical_last_summary_bar_ < 0) {
            late_historical_last_summary_bar_ = bar_index;
        }
        if ((bar_index - late_historical_last_summary_bar_) >= late_historical_summary_interval_bars_) {
            LOG_WARN("[ZONE_MODE] Feed summary: ignored " << late_historical_drop_count_
                     << " late HISTORICAL bars after LIVE activation"
                     << " (last " << late_historical_summary_interval_bars_ << " bars)");
            late_historical_drop_count_ = 0;
            late_historical_last_summary_bar_ = bar_index;
        }
    }
    ...
    if (trade_mgr.is_in_position() && !bar_history.empty()) {
        manage_position();
    ...
    // IMPORTANT: HISTORICAL bars are bootstrap phase; do not run incremental detection.
    // This keeps zone detection one-time during bootstrap and prevents count inflation.
    if (!historical_mode_ && bar_history.size() >= 100) {
        process_zones();
    }
        ...
        if (metadata.empty()) {
            ...
            if (check_bootstrap_validity(metadata)) {
                ...
                if (zone_persistence_.load_zones(active_zones, loaded_id)) {
                    if (active_zones.empty()) {
                        LOG_WARN("⚠️  Cached zones file loaded but contains 0 zones - regenerating");
                        active_zones.clear();
                    } else {
...
void LiveEngine::run(int duration_minutes) {
    ...
    LOG_INFO("[ZONE_MODE] HISTORICAL bootstrap mode active: one-time zone detection only");
    LOG_INFO("[ZONE_MODE] Incremental/periodic zone detection will start only after LIVE mode activation");
    ...
    while (true) {
        ...
        try {
            ...
            // Only run in LIVE mode. HISTORICAL bars are bootstrap and must not
            // trigger repeated zone detection scans.
            if (!historical_mode_ && config.live_zone_detection_interval_bars > 0) {
...
void LiveEngine::stop() {
    ...
    if (trade_mgr.is_in_position()) {  // ⭐ Use TradeManager
        ...
        double current_price = bar_history.empty() ? 0.0 : bar_history.back().close;
        std::string current_time = bar_history.empty() ? ""  : bar_history.back().datetime;
