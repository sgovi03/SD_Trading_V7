// ============================================================
// SD TRADING V8 - SYSTEM CONFIG LOADER (V8 PATCHED)
...
// ============================================================
// Private: load_v8_section
// Parses the "v8" JSON object into config.
// Safe no-op when v8_json is empty.
// ============================================================

void SystemConfigLoader::load_v8_section(const std::string& v8_json,
                                          SystemConfig& config)
{
    if (v8_json.empty()) {
    ...
    std::string sr = extract_json_string(v8_json, "symbol_registry_file");
    if (!sr.empty()) config.symbol_registry_file = sr;

    std::string cr = extract_json_string(v8_json, "config_root_dir");
    if (!cr.empty()) config.config_root_dir = cr;

    std::string dbp = extract_json_string(v8_json, "sqlite_db_path");
    if (!dbp.empty()) config.sqlite_db_path = dbp;

    std::string sch = extract_json_string(v8_json, "sqlite_schema_path");
    if (!sch.empty()) config.sqlite_schema_path = sch;

    config.scanner_mode_enabled     = extract_json_bool(v8_json, "scanner_mode_enabled");
    config.scanner_signal_min_score = extract_json_double(v8_json, "scanner_signal_min_score");

    std::string rank = extract_json_string(v8_json, "scanner_rank_by");
    if (!rank.empty()) config.scanner_rank_by = rank;

    std::string be = extract_json_string(v8_json, "compute_backend");
    if (!be.empty()) config.compute_backend = be;

    int pool = extract_json_int(v8_json, "cpu_thread_pool_size");
    if (pool > 0) config.cpu_thread_pool_size = pool;

    config.cuda_device_id = extract_json_int(v8_json, "cuda_device_id");

    int thresh = extract_json_int(v8_json, "cuda_min_symbols_threshold");
    ...
    LOG_INFO("[V8] SQLite schema:    " << config.sqlite_schema_path);
    LOG_INFO("[V8] Scanner mode:     " << (config.scanner_mode_enabled ? "YES" : "NO"));
    LOG_INFO("[V8] Min score:        " << config.scanner_signal_min_score);
    LOG_INFO("[V8] Rank by:          " << config.scanner_rank_by);
    LOG_INFO("[V8] Compute backend:  " << config.compute_backend
             << " (pool=" << config.cpu_thread_pool_size
             << ", cuda_id=" << config.cuda_device_id << ")");
}
    ...
    const SystemConfig& config, const std::string& filepath)
{
    ...
    file << "  \"order_submitter\": {\n";
    file << "    \"enabled\": "               << (config.order_submitter_enabled ? "true" : "false") << ",\n";
    file << "    \"base_url\": \""            << config.order_submitter_base_url       << "\",\n";
    file << "    \"long_strategy_number\": "  << config.order_submitter_long_strategy  << ",\n";
    file << "    \"short_strategy_number\": " << config.order_submitter_short_strategy << ",\n";
    file << "    \"timeout_seconds\": "       << config.order_submitter_timeout        << "\n";
    file << "  },\n\n";
